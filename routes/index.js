...
router.get('/create_file', function(req, res, next) {
    ...
    {
        res.status(404).json({error: 'User is null'});return;
    }
...
router.get('/add_payment', function(req, res, next) {
    ...
    var fs = require('fs');    
    fs.readFile("../public/files/"+id+"/"+file, {encoding: 'utf-8'}, function(err, data) {
        if (err) {
            res.status(500).json(err);return;
        }
        ...
        {
            res.status(500).json({message: 'Une erreur est survenue'});return;
        }
        ...
        fs.writeFile("../public/files/"+id+"/"+file, JSON.stringify(json), function(err2) {
            if (err2) {
                res.status(500).json(err2);return;
            }
